...
int main(int argc, char **argv) {
    ...
    // Improved efficiency by precomputing frequently used values
    float filmAspectRatio = filmApertureWidth / filmApertureHeight;
    float deviceAspectRatio = imageWidth / (float)imageHeight;
    float top = ((filmApertureHeight * inchToMm / 2) / focalLength) * nearClippingPlane;
    float right = ((filmApertureWidth * inchToMm / 2) / focalLength) * nearClippingPlane;

    // Precompute scale factors outside the loop
    float xscale = 1;
    float yscale = 1;
    // Fit film computation
    // ... [fit film computation code remains unchanged]

    // Using auto for type inference
    auto bottom = -top;
    auto left = -right;
    
    // Precompute inverse of the camera matrix
    Matrix44f worldToCamera = cameraToWorld.inverse();

    // Loop optimization
    for (uint32_t i = 0; i < numTris; ++i) {
        // ... [inner loop code remains unchanged, using precomputed worldToCamera]
    }
    ofs.close();
